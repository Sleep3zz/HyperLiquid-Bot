 ...
 * Tests many combinations to find the best settings
 */
...
console.log('╔══════════════════════════════════════════════════════════════════╗');
console.log('║     SOL-Specific Extensive Parameter Optimization               ║');
console.log('╚══════════════════════════════════════════════════════════════════╝');

const values = loadData();
console.log(`\n📊 Loaded ${values.length} data points for SOL\n`);

const results = [];

// Test 1: RSI variations (reversion mode)
for (const rsiL of [20, 25, 30, 35, 40, 45, 50]) {
  for (const rsiS of [55, 60, 65, 70, 75, 80, 85]) {
    for (const adx of [10, 15, 20, 25, 30, 35, 40]) {
      for (const sl of [1, 1.5, 2, 2.5, 3, 4]) {
...
if (profitable.length === 0) {
  ...
  profitable.slice(0, 30).forEach((r, i) => {
...
if (profitable.length > 0) {
  ...
  const adxCounts = {};
  profitable.forEach(r => { adxCounts[r.cfg.adx] = (adxCounts[r.cfg.adx] || 0) + 1; });
  const bestAdx = Object.entries(adxCounts).sort((a, b) => b[1] - a[1])[0];
  console.log(`   • Most common profitable ADX: ${bestAdx[0]} (${bestAdx[1]} configs)`);
}

// Save results
const outDir = path.join(__dirname, '..', 'backtest-results');
if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
const file = path.join(outDir, `SOL-extensive-optimization-${Date.now()}.json`);
