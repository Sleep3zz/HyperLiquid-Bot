 ...
 * Comprehensive search for best BTC parameters
 */
...
console.log('╔══════════════════════════════════════════════════════════════════╗');
console.log('║     BTC-Specific Extensive Parameter Optimization               ║');
console.log('╚══════════════════════════════════════════════════════════════════╝');

const values = loadData();
console.log(`\n📊 Loaded ${values.length} data points for BTC\n`);

const results = [];
let tested = 0;

// Test reversion mode - wide range
for (const rsiL of [20, 25, 30, 35, 40, 45, 50]) {
  for (const rsiS of [55, 60, 65, 70, 75, 80]) {
    for (const adx of [10, 15, 20, 25, 30, 35]) {
      for (const sl of [1, 1.5, 2, 2.5, 3, 3.5]) {
        for (const tp of [2, 3, 4, 5, 6, 7, 8]) {
...
// Test breakout mode
for (const rsiL of [50, 55, 60, 65, 70]) {
  for (const rsiS of [30, 35, 40, 45, 50]) {
    for (const adx of [15, 20, 25, 30]) {
      for (const sl of [1.5, 2, 2.5, 3}) {
        for (const tp of [3, 4, 5, 6, 7]) {
...
// Find best reversion and breakout separately
...
const current = { mode: 'reversion', rsiLong: 40, rsiShort: 60, adx: 20, sl: 3, tp: 5 };
const currentResult = runBacktest(values, current);

console.log(`\n${'─'.repeat(70)}`);
console.log('📊 COMPARISON WITH CURRENT CONFIG:');
console.log('─'.repeat(70));
console.log(`   Current: RSI 40/60, ADX <20, SL 3%/TP 5%`);
console.log(`            ${currentResult.trades} trades, ${(currentResult.winRate*100).toFixed(1)}% win, +$${currentResult.netPnl.toFixed(2)}`);
console.log(`   Best:    RSI ${best.cfg.rsiLong}/${best.cfg.rsiShort}, ADX ${best.cfg.mode === 'reversion' ? '<' : '>='}${best.cfg.adx}, SL ${best.cfg.sl}%/TP ${best.cfg.tp}%`);
console.log(`            ${best.trades} trades, ${(best.winRate*100).toFixed(1)}% win, +$${best.netPnl.toFixed(2)}`);
const improvement = ((best.netPnl - currentResult.netPnl) / Math.abs(currentResult.netPnl) * 100);
console.log(`   Improvement: ${improvement >= 0 ? '+' : ''}${improvement.toFixed(0)}%`);

// Save
const outDir = path.join(__dirname, '..', 'backtest-results');
if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
const file = path.join(outDir, `BTC-extensive-optimization-${Date.now()}.json`);
fs.writeFileSync(file, JSON.stringify({ 
  timestamp: new Date().toISOString(),
  totalTested: tested,
  profitableCount: profitable.length,
  bestConfig: best,
  currentConfig: { config: current, results: currentResult },
